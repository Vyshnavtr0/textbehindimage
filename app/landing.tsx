...
const LandingPage = () => {
  ...
  const handleLogin = async () => {
    try {
      ...
      if (result.user) {
        // User successfully logged in, navigate to editor
        router.push('/editor');
      }
    } catch (error) {
      ...
      <div className="bg-white text-white p-4 flex justify-between items-center">
        ...
        {user ? (
         ...
         <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
          ...
          <Button 
            className="bg-white text-black px-4 py-2 sm:px-6 sm:py-4 rounded-md text-sm sm:text-xl font-semibold hover:text-white hover:bg-gray-800 transition-colors duration-300 shadow-lg"
            onClick={handleLogin}
          >
            Login with Google
          </Button>
